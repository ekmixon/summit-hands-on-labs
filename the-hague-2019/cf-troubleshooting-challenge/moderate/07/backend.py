...
@app.route("/")
def greeting():
   return "<h1 allign=center>This is test application, backend part<h1><p>App will curl AWS and GCP status page and check, if services available</p><p>/aws_eu, /gcp, /status calls available</p>"
...
def aws_status():
   ...
   if status_aws_europe := status_aws_europe_table.xpath(
       './/td[@colspan]')[0].text:
